	...
	if(strlen(trueGraphFName)==0)
	{
		while(currK<=maxMBSizeApprox)
		{
			...
			while(notConverged)
			{
				...
				cout <<"Current "<< currK <<" Score " << currGlobalScore << endl;
			}
			currK++;
		}
...
MetaLearner::collectMoves_Orthogroups(int currK)
{
	...
	//				continue;
				...
				{	
					...
					for(map<string,map<string,STRINTMAP*>*>::iterator rIter=speciesTFSet.begin();rIter!=speciesTFSet.end();rIter++)
					{
						...
						if(strcmp(specIter->first.c_str(),"Scer")==0)
						{
							scertfs[reg->getName()]=0;
						}
		...
		for(INTINTMAP_ITER csIter=cset->begin();csIter!=cset->end();csIter++)
		{
			...
			VariableManager* vMgr=sdm->getVariableManager();
			VSET& varSet=vMgr->getVariableSet();
			cout <<"CurrK: "<< currK << " Found edge for "<<csIter->first<<" " << varSet[tfid]->getName().c_str() << "<->" 
			<< varSet[tgtid]->getName().c_str() << " score deltas" << bestscoreImprovement_PerSpecies[csIter->first] << " in " <<speciesIDNameMap[csIter->first] << endl;
	...
	cout <<"Total Scer TFs considered" << scertfs.size() << endl;
	ofstream oFile("scertfs_ogs.txt");
	for(map<string,int>::iterator aIter=scertfs.begin();aIter!=scertfs.end();aIter++)
	{
		oFile << aIter->first << endl;
	}
	oFile.close();
	return 0;
}
...
{
	PotentialManager* potMgr=speciesDataSet[speciesName]->getPotentialManager();
	EvidenceManager* evMgr=speciesDataSet[speciesName]->getEvidenceManager();
	VariableManager* varMgr=speciesDataSet[speciesName]->getVariableManager();
	VSET& varSet=varMgr->getVariableSet();	
	string mbkey;
	char mbchar[256];
	...
	if(strcmp(speciesName.c_str(),"Cgla")==0)
	{
		//cout << "Learnig param of " << varSet[sFactor->fId]->getName() << endl;
	}
	...
	paramCnt=paramCnt+((varCnt*(varCnt-1))/2);
	//paramCnt=paramCnt+((varCnt*(varCnt-1))/2);
	double pll=unreg_pll-(0.5*paramCnt*log(evMgr->getTestSet().size()));
...
MetaLearner::showModelParameters(int foldid)
{
	...
	for(map<string,SpeciesDataManager*>::iterator eIter=speciesDataSet.begin();eIter!=speciesDataSet.end();eIter++)
	{
		...
		for(map<int,SlimFactor*>::iterator sIter=slimFactorSet.begin();sIter!=slimFactorSet.end();sIter++)
		{
			...
			for(INTDBLMAP_ITER dIter=mbwt.begin();dIter!=mbwt.end();dIter++)
			{
				if(dIter!=mbwt.begin())
				{
					oFile <<",";
		...
		while(iter<(sampleCnt+burnin))
		{
			...
			for(map<string,int>::iterator vIter=subgraphVarSet.begin(); vIter!=subgraphVarSet.end();vIter++)
			{
				...
				if((isnan(sample_s))||(isinf(sample_s)))
				{
					...
					{
						cout << " "<< mbIter->first <<"=" << initialSample[mbIter->first];
					}
					cout << endl;
				}
