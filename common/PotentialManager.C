...
PotentialManager::init()
{
    ...
    {
        int trainEvidSetSize=evMgr->getTestSetSize();
        estimateAllMeanCov_Eff_onefold(trainEvidSetSize);
    }
    ludecomp=gsl_matrix_alloc(MAXFACTORSIZE_ALLOC,MAXFACTORSIZE_ALLOC);
    perm=gsl_permutation_alloc(MAXFACTORSIZE_ALLOC);
    return 0;
...
int
PotentialManager::estimateAllMeanCov_Eff_onefold(int trainEvidSetSize)
{
...
PotentialManager::estimateCovariance_Eff(int uId, int vId)
{
    ...
    //Now estimate the variance
    double var=0; //(*vcov)[uId]=(0.001+ssd)/((double)(evidCnt-1));
    if(uId==vId)
    {
...
PotentialManager::getPseudoLikelihood(SlimFactor* sFactor,VSET& varSet, bool train)
{
    ...
    double variance=(0.001+ssd)/(dataSetSize-1.0);  //(0.001+ssd)/((double)(data->getColCnt()-1))
    ...
    for(int j=id+1;j<data->getRowCnt();j++)
    {
        ...
        //cout << "cov[" << id << "," << j << "]=" << covMat->getValue(id,j) << endl;
    }
    //cout << "PotentialManager::getPseudoLikelihood sFactor="<<sFactor->fId << " pll=" << pll << endl;
    return pll;
}
/*
double
PotentialManager::getPseudoLikelihood(SlimFactor* sFactor,VSET& varSet, bool train)
{
    ...
    for(INTINTMAP_ITER dIter=dataSet->begin();dIter!=dataSet->end();dIter++)
    {
        ...
        for(auto vIter=sFactor->mergedMB.begin();vIter!=sFactor->mergedMB.end(); vIter++)
        {
            int vId=*vIter;
            Evidence* evid=(*evidMap)[vId];
            double val=evid->getEvidVal();
...
double
PotentialManager::computePotentialMBCovMean(SlimFactor* sFactor, int& status, int datasize)//unordered_map<int,double>& mbcondMean_Vect)
{
    ...
    //cout << "my covariance matrix is " << endl;
    ...
    double jointll1 = computeLL_Tracetrick(ParChildID.size(),determinant,datasize);
    double jointll2 = computeLL_Tracetrick(mbsize,determinantP,datasize);
    double pll = jointll1 - jointll2;
    //cout <<"Target=" << sFactor->fId << " dataSize=" << datasize << " determinant=" << determinant << " dim=" << ParChildID.size() << " determinantParent=" << determinantP <<  " dimP=" <<mbsize << " var(target)=" <<covMat->getValue(vId,vId) <<" jointll1=" << jointll1 <<" jointll2=" << jointll2<<endl;
    if(isinf(pll)){
        status=-1;
...
double
PotentialManager::computeLL_Tracetrick(int dim, double determinant, int datasize)  //sampleSize=testdataSize;
{
    double testdataSize=datasize;
